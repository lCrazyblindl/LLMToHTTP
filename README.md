...
4. **Declarative queries match code, without running code.** An OData/GraphQL-style `query` variant ties `code_exec` on every task (both collapse the result bucket), but needs no code sandbox. For work the query DSL can express, that's the safer way to get the same savings; arbitrary computation it can't express is where code execution (or a richer protocol) earns its place.
