...
- **v0.4 real-tool checks** ([`docs/GENERATORS.md`](docs/GENERATORS.md), [`docs/MCP-SERVERS.md`](docs/MCP-SERVERS.md), [`validation-real.md`](experiments/token-bench/validation-real.md)) — measuring *real* tools, not our own: three real OpenAPI→MCP generators and three real published MCP servers all leave the compact-menu savings (~76–90%) on the table, and a live end-to-end run on the hosted Swagger Petstore found the naive menu *less* reliable than compact (0/3 vs 3/3 on a count task).
